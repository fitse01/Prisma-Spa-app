...
const createEmployee = async (req, res) => {
    try {
      ...
      const { full_name, email, phone_no, position, experience_years, personal_info, skills, employment_status, role, social_media } = req.body;
      ...
      const employee = await prisma.employee.create({
        data: {
          ...
          personal_info,
          skills: JSON.parse(skills),  // Assuming skills are sent as JSON array
          employment_status,
          role,
          social_media: JSON.parse(social_media) // Assuming social media links are sent as JSON array
        }
      });
  ...
  const updateEmployee = async (req, res) => {
    ...
    try {
      ...
      if (experience_years !== undefined) updatedData.experience_years = parseInt(experience_years);
      if (personal_info !== undefined) updatedData.personal_info = personal_info;
      if (skills !== undefined) updatedData.skills = skills;
      if (employment_status !== undefined) updatedData.employment_status = employment_status;
...
const toggleEmployeeStatus = async (req, res) => {
  ...
  try {
    ...
    const updatedEmployee = await prisma.employee.update({
      where: { id: parseInt(id) },
      data: { employment_status: employee.employment_status === 'Active' ? 'On Leave' : 'Active' }
    });
    res.json(updatedEmployee);
  } catch (error) {
